"""Experimental Cu-PAN Dose Calibration Model Training and Validation Script.

Fits piecewise monotonic interpolation and polynomial models for Cu-PAN, computes
...
def run_dose_model_training():
    ...
    print("=" * 60)
    print("  Cu-PAN Strip Calibration Model Training & Validation")
    ...
    report_mono = compute_validation_metrics(doses, pred_mono, "Cu-PAN Piecewise Monotonic Spline")

    print("\n--- Model Performance: Cu-PAN Piecewise Monotonic Spline ---")
    ...
    dense_delta = np.linspace(0.0, 75.0, 200)
    dense_dose = np.array([dataset.model.predict(d)[0] for d in dense_delta])

    axes[0].plot(dense_delta, dense_dose, color="#0284c7", linewidth=2.5, label="Cu-PAN Piecewise Fit")
    axes[0].scatter(deltas, doses, color="#e11d48", s=60, zorder=5, label="Chamber Data (25°C, 50% RH)")
    axes[0].set_title("Cu-PAN H₂S Chemical Response Curve", fontsize=12, fontweight="bold")
    axes[0].set_xlabel("Optical Shift ΔE₀₀ (ISO/CIE 11664-6)", fontsize=10)
    axes[0].set_ylabel("Cumulative Dose (ppm·h)", fontsize=10)
    ...
    axes[1].set_title("Cu-PAN Experimental Validation Parity", fontsize=12, fontweight="bold")
    axes[1].set_xlabel("True Chamber Dose (ppm·h)", fontsize=10)
    axes[1].set_ylabel("Estimated Model Dose (ppm·h)", fontsize=10)
    axes[1].grid(True, alpha=0.3)
    axes[1].legend(frameon=True)

    plt.tight_layout()
    plot_path = OUTPUT_DIR / "cupan_dose_calibration_kinetics.png"
    plt.savefig(plot_path)
    plt.close()

    print(f"\n[+] Saved Cu-PAN validation kinetics plot to: {plot_path}")
