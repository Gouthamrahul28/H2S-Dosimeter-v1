/**
 * shared/colorimetryEngine.js
 * 
 * Standardized, Chemistry-Agnostic Colorimetric Core Engine.
 ...
 * This module contains ZERO chemistry assumptions, ZERO reagent thresholds,
 * ZERO gas PPM calculations, and ZERO baseline coordinates.
 * It is pure, reproducible optical and colorimetric science.
 */

// CIE Standard Illuminant D65 Reference White Point (2° Standard Observer)
export const D65_WHITE = Object.freeze({
  x: 0.95047,
  y: 1.00000,
  z: 1.08883
});
...
export const DEFAULT_CCM = Object.freeze([
...
export const M_BRAD = Object.freeze([
  [ 0.8951,  0.2664, -0.1614 ],
  [-0.7502,  1.7135,  0.0367 ],
  [ 0.0389, -0.0685,  1.0296 ]
]);

export const M_BRAD_INV = Object.freeze([
 ...
 * Decompresses non-linear gamma-encoded 8-bit channel [0, 255] to linear reflectance [0.0, 1.0].
 * 
 * @param {number} c - 8-bit sRGB channel intensity (0-255)
 * @returns {number} Linear radiometric intensity [0.0, 1.0]
 */
 ...
 * Compresses linear radiometric reflectance [0.0, 1.0] to non-linear 8-bit sRGB [0, 255].
 * 
 * @param {number} cLin - Linear reflectance intensity [0.0, 1.0]
 * @returns {number} Clamped 8-bit sRGB channel integer [0, 255]
 */
...
/**
 * Linearizes an sRGB object { r, g, b } (values 0-255) to { r, g, b } (0.0-1.0).
 */
...
/**
 * Converts a linear RGB object { r, g, b } (0.0-1.0) to sRGB { r, g, b } (0-255).
 */
export function linearToSrgbRgb(linRgb) {
  return {
 ...
 * Transforms linearized camera device RGB signals to device-independent CIE 1931 XYZ coordinates.
 * 
 * @param {number} rLin - Linear red channel [0.0, 1.0]
 * @param {number} gLin - Linear green channel [0.0, 1.0]
 * @param {number} bLin - Linear blue channel [0.0, 1.0]
 * @param {number[][]} ccm - 3x3 Color Correction Matrix
 * @returns {{ x: number, y: number, z: number }} CIE XYZ tristimulus values
 */
 ...
 * Transforms CIE XYZ tristimulus values from a source capture white point to target white point (D65).
 * 
 * @param {{ x: number, y: number, z: number }} xyz - Source XYZ coordinates
 * @param {{ x: number, y: number, z: number }} srcWhite - Measured source illuminant white point XYZ
 * @param {{ x: number, y: number, z: number }} tgtWhite - Target white point XYZ (default D65)
 * @returns {{ x: number, y: number, z: number }} Chromatically adapted XYZ coordinates
 */
...
/**
 * Converts CIE 1931 XYZ tristimulus values to CIE 1976 CIELAB (L*, a*, b*).
 * 
 * @param {number} x - CIE X tristimulus
 * @param {number} y - CIE Y tristimulus
 * @param {number} z - CIE Z tristimulus
 * @param {{ x: number, y: number, z: number }} whitePoint - Reference illuminant white point (D65)
 * @returns {{ L: number, a: number, b: number }} CIELAB coordinates
 */
...
/**
 * Converts CIE 1976 CIELAB (L*, a*, b*) back to CIE 1931 XYZ.
 * 
 * @param {number} L - Lightness [0, 100]
 * @param {number} a - Red-Green chromatic coordinate
 * @param {number} b - Yellow-Blue chromatic coordinate
 * @param {{ x: number, y: number, z: number }} whitePoint - Reference illuminant white point (D65)
...
/**
 * Converts CIE 1976 CIELAB (L*, a*, b*) directly to display sRGB { r, g, b }.
 */
 ...
 * Implementation includes standard Sharma et al. (2005) numerical safeguards against atan2 singularities.
 * 
 * @param {{ L: number, a: number, b: number }} lab1 - Reference colour
 * @param {{ L: number, a: number, b: number }} lab2 - Sample colour
 * @returns {number} Perceptual colour difference ΔE₀₀
 */
...
/**
 * Helper to convert Hex to RGB { r, g, b }.
 */
